...
function cargarDetallesVentasPorDoc(codDoc){
	var fechaVentas = $("#inpFecha").val();
	$.ajax({
		data: ({codDocPHP: codDoc,fechaVentasPHP: fechaVentas}),
		dataType: 'json',
		type: 'POST',
		timeout: 60000,
...
function cargarDetallesVentasPorVendedorDelMes(rutVendedor, nombreVendedor){
	...
	$.ajax({
		async: false,
		data: ({mesVentasPHP: mes, rutVendedorPHP: rutVendedor}),
		dataType: 'json',
		type: 'POST',
		timeout: 60000,
		...
		$.ajax({
			...
			success: function(data){
				...
				if (data.length != 0) {
					...
					for (var i = 1; i <= 31; i++) {
						j++;
						if (j < 10 && String(j).length == 1) { j = "0" + j}
						while(j != data[i-1][0]){
...
function establecerVentaPagada(){
	$("#btnPagarVenta").click(function(){
		if ($("#inpNumeroDocPago").val() > 0 && $("#inpNumeroDocPago").val() != "" && $("#selDocPago").val() !== '1') {
			...
			if ($("#inpNumeroDocPago").val() < 0) {alert("CORRIJA EL NÚMERO DE DOCUMENTO DE PAGO.");}
			if ($("#inpNumeroDocPago").val() == "") {alert("DEBE INGRESAR EL NÚMERO DE DOCUMENTO DE PAGO.");}
			if ($("#selDocPago").val() == '1') {alert("DEBE INGRESAR EL DOCUMENTO DE PAGO.");}
		}
	});
}
